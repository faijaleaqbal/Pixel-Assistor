...
module.exports = {
  ...
  async execute(message, args, client) {
    const q = args.join(' ');
    if (!q) return message.reply({ embeds: [responseBuilder.buildResult({ description: 'Ask a question!'})] });
    ...
    return message.reply({ embeds: [responseBuilder.buildResult({ title: '🎱 Magic 8-Ball', fields: [{ name: 'Question', value: trimmedQ, inline: false },
