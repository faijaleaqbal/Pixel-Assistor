...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    for (const input of roleInputs) {
      ...
      if (!role) return message.reply({ embeds: [responseBuilder.buildResult({ description: `Role not found: ${input}`})] });
    ...
    try {
      ...
      return message.reply({ embeds: [responseBuilder.buildResult({ description: `Failed to persist roles: ${e.message}`})] });
    }
    return message.reply({ embeds: [responseBuilder.buildResult({ description: `Persisted ${roleIds.length} role(s) on ${target.user.tag}.`})] });
  },
};
