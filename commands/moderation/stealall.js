...
module.exports = {
  ...
  async execute(message) {
    ...
    if (!target) return message.reply({ embeds: [new EmbedBuilder().setColor(0xED4245).setDescription('No message found.')] });

    const custom = [...new Set((target.content.match(/<a?:[\w]+:\d+>/g) || []))];
    if (!custom.length) return message.reply({ embeds: [new EmbedBuilder().setColor(0xED4245).setDescription('No custom emojis found.')] });
    ...
    if (message.guild.emojis.cache.size >= maxEmojis)
      return message.reply({ embeds: [new EmbedBuilder().setColor(0xED4245).setDescription('Server emoji limit reached.')] });
    ...
    return message.reply({ embeds: [new EmbedBuilder().setColor(0x57F287).setTitle('Steal Results').setDescription(results.join('\n'))] });
  },
};
