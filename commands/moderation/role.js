...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    if (sub === 'all') {
      ...
      if (!role) return message.reply({ embeds: [responseBuilder.buildResult({ description: 'Mention a role or provide its name/ID.'})] });
      const roleCheck = canManageRole(message.member, role, message.guild, { actionName: 'assign' });
      if (!roleCheck.ok) return message.reply({ embeds: [responseBuilder.buildResult({ description: `❌ ${roleCheck.error}`})] });
      ...
      if (!humans.size) return message.reply({ embeds: [responseBuilder.buildResult({ description: 'All human members already have this role.'})] });

      const confirm = await message.reply({ embeds: [responseBuilder.buildResult({ description: `Add ${role} to **${humans.size}** human members? Type \`yes\` to confirm.`})] });
      ...
      collector.on('collect', async () => {
        ...
        await confirm.edit({ embeds: [responseBuilder.buildResult({ description: `✅ Added ${role} to ${count} member(s).`})] });
        activeBatches.delete(message.guild.id);
      });
      collector.on('end', (collected, reason) => {
        if (reason === 'time' && !collected.size) {
          confirm.edit({ embeds: [responseBuilder.buildResult({ description: 'Timed out.'})] }).catch(() => {});
          activeBatches.delete(message.guild.id);
        }
      });
      return;
    ...
    if (sub === 'bots') {
      ...
      if (!bots.size) return message.reply({ embeds: [responseBuilder.buildResult({ description: 'All bots already have this role.'})] });

      let count = 0;
      for (const [, m] of bots) {
        try { await m.roles.add(role); count++; } catch { /* skip */ }
      }
      return message.reply({ embeds: [responseBuilder.buildResult({ description: `✅ Added ${role} to ${count} bot(s).`})] });
    }
    ...
    if (sub === 'humans') {
      ...
      try { await message.guild.members.fetch(); } catch { /* ignore */ }
      const humans = message.guild.members.cache.filter(m => !m.user.bot && !m.roles.cache.has(role.id));
      if (!humans.size) return message.reply({ embeds: [responseBuilder.buildResult({ description: 'All humans already have this role.'})] });

      let count = 0;
      for (const [, m] of humans) {
        try { await m.roles.add(role); count++; } catch { /* skip */ }
      }
      return message.reply({ embeds: [responseBuilder.buildResult({ description: `✅ Added ${role} to ${count} member(s).`})] });
    }
    ...
    if (sub === 'create') {
      ...
      if (!name) return message.reply({ embeds: [responseBuilder.buildResult({ description: 'Provide a role name.'})] });
      ...
      try {
        ...
        return message.reply({ embeds: [responseBuilder.buildResult({ description: `✅ Role ${role} created.`})] });
      } catch (e) {
        return message.reply({ embeds: [responseBuilder.buildResult({ description: `Failed to create role: ${e.message}`})] });
      }
    }
    ...
    if (sub === 'delete') {
      ...
      try {
        ...
        return message.reply({ embeds: [responseBuilder.buildResult({ description: `Failed to delete role: ${e.message}`})] });
      }
    }
    ...
    if (sub === 'rename') {
      ...
      if (!newName) return message.reply({ embeds: [responseBuilder.buildResult({ description: 'Provide a new name.'})] });
      ...
      try {
        ...
        return message.reply({ embeds: [responseBuilder.buildResult({ description: `✅ Role renamed to ${role}.`})] });
      } catch (e) {
        return message.reply({ embeds: [responseBuilder.buildResult({ description: `Failed to rename role: ${e.message}`})] });
      }
    }
    ...
    if (sub === 'temp') {
      ...
      if (!targetCheck.ok) return message.reply({ embeds: [responseBuilder.buildResult({ description: `❌ ${targetCheck.error}`})] });
      ...
      if (!durationStr) return message.reply({ embeds: [responseBuilder.buildResult({ description: 'Provide a duration (e.g. 10m, 1h, 1d).'})] });
      const durMs = parseDuration(durationStr);
      if (!durMs) return message.reply({ embeds: [responseBuilder.buildResult({ description: 'Invalid duration format.'})] });
      if (durMs > 7 * 86400000) return message.reply({ embeds: [responseBuilder.buildResult({ description: 'Maximum duration is 7 days.'})] });
      ...
      try {
        ...
        return message.reply({ embeds: [responseBuilder.buildResult({ description: `✅ Added ${role} to ${target.user.tag} for \`${durationStr}\`.`})] });
      } catch (e) {
        return message.reply({ embeds: [responseBuilder.buildResult({ description: `Failed to add role: ${e.message}`})] });
      }
    }
    ...
    if (sub === 'cancel') {
      ...
      if (!batch) return message.reply({ embeds: [responseBuilder.buildResult({ description: 'No active batch operation.'})] });
      batch.collector.stop('cancelled');
      batch.message.edit({ embeds: [responseBuilder.buildResult({ description: 'Batch operation cancelled.'})] }).catch(() => {});
      activeBatches.delete(message.guild.id);
      return message.reply({ embeds: [responseBuilder.buildResult({ description: '✅ Batch cancelled.'})] });
    }
    ...
    if (sub === 'status') {
      const batch = activeBatches.get(message.guild.id);
      if (!batch) return message.reply({ embeds: [responseBuilder.buildResult({ description: 'No active batch operation.'})] });
      return message.reply({ embeds: [responseBuilder.buildResult({ title: 'Active Batch Operation', description: `**Type:** ${batch.type}\n**Role:** ${batch.role}`})] });
    }
    ...
    if (!roleInputs.length) return message.reply({ embeds: [responseBuilder.buildResult({ description: 'Provide up to 5 roles (comma-separated).'})] });
    ...
    return message.reply({ embeds: [responseBuilder.buildResult({ title: 'Roles updated', description: results.join('\n')})] });
  },
};
