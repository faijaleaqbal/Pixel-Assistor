...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    if (!name || !url) return message.reply(opts(buildContainer({ description: 'Usage: `steal <name> [url]` — attach an image or pass a URL.' })));
    try {
      if (/\.gif/.test(url)) {
        const e = await message.guild.emojis.create({ attachment: url, name });
        return message.reply(opts(responseBuilder.buildResult({ description: `✅ Stole emoji ${e} (\`${name}\`)`})));
      }
      const e = await message.guild.emojis.create({ attachment: url, name });
      return message.reply(opts(responseBuilder.buildResult({ description: `✅ Stole emoji ${e} (\`${name}\`)`})));
    } catch (e) {
