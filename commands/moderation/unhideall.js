...
module.exports = {
  ...
  async execute(message) {
    ...
    if (!channels.size) return message.reply({ embeds: [new EmbedBuilder().setColor(0xFEE75C).setDescription('No hidden text channels found.')] });
    let count = 0;
    ...
    return message.reply({ embeds: [new EmbedBuilder().setColor(0x57F287).setDescription(`👁️ Unhid ${count} text channel(s).`)] });
  },
};
