...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    if (sub === 'remove') {
      ...
      if (!id) return message.reply(opts(responseBuilder.buildResult({ description: 'Provide a warning ID. Use `?warn list <@user|userID>` to see IDs.'})));
      return message.reply(opts(responseBuilder.buildResult({ description: 'Individual warning removal is not supported yet. Use `?clearwarns <@user|userID>` to clear all warnings for a user.'})));
    }
    ...
    if (sub === 'clearall') {
      if (!hasPermission(message.member, 'Administrator')) {
        return message.reply(opts(responseBuilder.buildResult({ description: 'Only administrators can clear all server warnings.'})));
      }
      try {
        const total = await db.warn.clearGuild(message.guild.id);
        return message.reply(opts(responseBuilder.buildResult({ description: `✅ Cleared ${total} warning(s) across all members.`})));
      } catch {
        return message.reply(opts(responseBuilder.buildResult({ description: 'Failed to clear guild warnings.'})));
      }
    }
    ...
    if (target.user.bot) {
      return message.reply(opts(responseBuilder.buildResult({ description: '❌ You cannot warn a bot.'})));
    }
    ...
    return message.reply(opts(responseBuilder.buildResult({ description: `⚠️ ${target.user.tag} has been warned.\n**Reason:** ${reason}`})));
  },
};
