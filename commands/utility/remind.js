...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (!target) return;
    if (target.bot) {
      return message.reply({ embeds: [new EmbedBuilder().setColor(0xED4245).setDescription('Cannot send a reminder to a bot.')] });
    }
    if (target.id === message.author.id) {
      return message.reply({ embeds: [new EmbedBuilder().setColor(0xED4245).setDescription('You can\'t remind yourself. Use `?rm <duration> [reason]` instead.')] });
    }
    ...
    const dmEmbed = new EmbedBuilder()
      .setColor(0x5865F2)
      .setTitle('\U0001F4DD You\'ve been reminded!')
      .addFields(
    ...
    const successEmbed = new EmbedBuilder()
      .setColor(0x57F287)
      .setDescription(`\u2705 Reminded ${target.username} successfully.`);
