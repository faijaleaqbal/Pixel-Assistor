...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    if (sub === 'reminders') {
      ...
      if (!mine || !mine.length) return message.reply({ embeds: [responseBuilder.buildResult({ description: 'You have no reminders.'})] });
    ...
    if (sub === 'upi') {
      ...
      if (!rows.length) return message.reply({ embeds: [responseBuilder.buildResult({ description: 'No saved UPI IDs.'})] });
    ...
    if (sub === 'warns') {
      ...
      if (target.id !== message.author.id && !hasPermission(message.member, 'ModerateMembers')) {
        return message.reply({ embeds: [responseBuilder.buildResult({ description: "You need ModerateMembers to view others' warns."})] });
      }
      const warns = await getDb().warn.list(target.id, message.guild.id);
      if (!warns.length) return message.reply({ embeds: [responseBuilder.buildResult({ description: 'No warns on record.'})] });
      ...
      return message.reply({ embeds: [responseBuilder.buildResult({ title: '\u26A0\uFE0F Warns — ' + target.tag, fields: [fields]})], allowedMentions: { parse: [] } });
    }

    // ── admins ──
    if (sub === 'admins') {
      const admins = message.guild.members.cache.filter(m => m.permissions.has(PermissionsBitField.Flags.Administrator) && !m.user.bot);
      if (!admins.size) return message.reply({ embeds: [responseBuilder.buildResult({ description: 'No admin members found.'})] });
    ...
    if (sub === 'mods') {
      ...
      if (!mods.size) return message.reply({ embeds: [responseBuilder.buildResult({ description: 'No mod members found.'})] });
    ...
    if (sub === 'bots') {
      ...
      if (!bots.size) return message.reply({ embeds: [responseBuilder.buildResult({ description: 'No bots in this server.'})] });
    ...
    if (sub === 'bans') {
      if (!hasPermission(message.member, 'BanMembers')) {
        return message.reply({ embeds: [responseBuilder.buildResult({ description: 'You need BanMembers permission.'})] });
      }
      try {
        const bans = await message.guild.bans.fetch();
        if (!bans.size) return message.reply({ embeds: [responseBuilder.buildResult({ description: 'No bans in this server.'})] });
        ...
        return message.reply({ embeds: [responseBuilder.buildResult({ description: 'Failed to fetch bans.'})] });
      }
    }
    ...
    if (sub === 'boosters') {
      ...
      if (!boosters.size) return message.reply({ embeds: [responseBuilder.buildResult({ description: 'No boosters in this server.'})] });
    ...
    if (sub === 'emojis') {
      ...
      if (!emojis.size) return message.reply({ embeds: [responseBuilder.buildResult({ description: 'No emojis in this server.'})] });
      const pages = [];
      let current = '';
    ...
    if (sub === 'botemojis') {
      const emojis = message.guild.emojis.cache;
      if (!emojis.size) return message.reply({ embeds: [responseBuilder.buildResult({ description: 'No custom emojis in this server.'})] });
    ...
    if (sub === 'roles') {
      ...
      if (!list) return message.reply({ embeds: [responseBuilder.buildResult({ description: 'No roles found.'})] });
    ...
    if (sub === 'inrole') {
      ...
      if (!role) return message.reply({ embeds: [responseBuilder.buildResult({ description: 'Mention a role.'})] });
      const members = role.members;
      if (!members.size) return message.reply({ embeds: [responseBuilder.buildResult({ description: 'No members with ' + role.name + '.'})] });
    ...
    if (sub === 'activedeveloper') {
      ...
      if (!devs.size) return message.reply({ embeds: [responseBuilder.buildResult({ description: 'No active developers found.'})] });
    ...
    if (sub === 'early') {
      ...
      if (!early.size) return message.reply({ embeds: [responseBuilder.buildResult({ description: 'No early supporters found.'})] });
    ...
    if (sub === 'joinpos') {
      ...
      if (!pos) return message.reply({ embeds: [responseBuilder.buildResult({ description: 'Member not found.'})] });
    ...
    return message.reply({ embeds: [responseBuilder.buildResult({ description: 'Usage: ' + BT + config.prefix + 'list <reminders|upi|warns|admins|mods|bots|bans|boosters|emojis|botemojis|roles|inrole|activedeveloper|early|joinpos|createpos>' + BT})] });
  },
};
