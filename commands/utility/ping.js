...
module.exports = {
  ...
  async execute(message) {
    ...
    const sent = await message.reply(opts(buildContainer({ description: '🏓 Pinging...' })));
    ...
    return sent.edit(opts(embed));
  },
  async slashExecute(interaction, client) {
    const wsLatency = Math.max(0, Math.round(client.ws.ping));
    const sent = await interaction.reply(opts(buildContainer({ description: '🏓 Pinging...' }), { fetchReply: true }));
