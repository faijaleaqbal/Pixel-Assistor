...
module.exports = {
  ...
  async execute(message, args) {
    const name = args[0];
    if (!name || name.length > 100) return message.reply({ embeds: [new EmbedBuilder().setColor(0xED4245).setDescription('Provide a role name (max 100 chars).')] });
    ...
    try {
      ...
      return message.reply({ embeds: [new EmbedBuilder().setColor(0x57F287).setDescription(`✅ Role **${role.name}** (\`${role.id}\`) created.`)] });
    } catch (e) {
