...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (sub === 'enable' || sub === 'on') {
      ...
      return message.reply({ embeds: [E(GREEN, '✅ Anti-nuke is now **enabled**.')] });
    }
    ...
    if (sub === 'disable' || sub === 'off') {
      cfg.enabled = false;
      await db.antinuke.set(gid, cfg);
      return message.reply({ embeds: [E(GREEN, '✅ Anti-nuke is now **disabled**.')] });
    }
    ...
    if (sub === 'setup') {
      ...
      await db.antinuke.set(gid, cfg);
      return message.reply({ embeds: [E(GREEN, '✅ Anti-nuke **enabled**. Log channel set to this channel. You have been added as an owner.')] });
    }
    ...
    if (sub === 'logging') {
      ...
      if (!ch) return message.reply({ embeds: [E(RED, 'Mention a channel to set as the log channel.')] });
      cfg.logChannel = ch.id;
      await db.antinuke.set(gid, cfg);
      return message.reply({ embeds: [E(GREEN, `✅ Log channel set to ${ch}.`)] });
    }

    // ── Punishment ──
    if (sub === 'punishment') {
      const type = args[1]?.toLowerCase();
      if (!['ban', 'kick', 'strip'].includes(type)) return message.reply({ embeds: [E(RED, 'Punishment must be `ban`, `kick`, or `strip`.')] });
      cfg.punishment = type;
      await db.antinuke.set(gid, cfg);
      return message.reply({ embeds: [E(GREEN, `✅ Punishment set to \`${type}\`.`)] });
    }
    ...
    if (sub === 'owner') {
      ...
      if (action === 'add') {
        ...
        if (!user) return;
        if (cfg.owners.includes(user.id)) return message.reply({ embeds: [E(YELLOW, `${user.username} is already an owner.`)] });
        cfg.owners.push(user.id);
        await db.antinuke.set(gid, cfg);
        return message.reply({ embeds: [E(GREEN, `✅ ${user.username} has been added as an owner.`)] });
      }
      ...
      if (action === 'remove') {
        ...
        await db.antinuke.set(gid, cfg);
        return message.reply({ embeds: [E(GREEN, `✅ ${user.username} has been removed from owners.`)] });
      }

      if (action === 'reset') {
        cfg.owners = [];
        await db.antinuke.set(gid, cfg);
        return message.reply({ embeds: [E(GREEN, '✅ All owners have been cleared.')] });
      }

      return message.reply({ embeds: [E(RED, 'Owner sub-commands: `add`, `remove`, `list`, `reset`.')] });
    }
    ...
    if (sub === 'whitelist') {
      ...
      if (action === 'add') {
        const user = await resolveUserArg(message, args[2]);
        if (!user) return;
        if (cfg.whitelist.includes(user.id)) return message.reply({ embeds: [E(YELLOW, `${user.username} is already whitelisted.`)] });
        cfg.whitelist.push(user.id);
        await db.antinuke.set(gid, cfg);
        return message.reply({ embeds: [E(GREEN, `✅ ${user.username} has been whitelisted.`)] });
      }
      ...
      if (action === 'remove') {
        ...
        await db.antinuke.set(gid, cfg);
        return message.reply({ embeds: [E(GREEN, `✅ ${user.username} has been removed from the whitelist.`)] });
      }

      if (action === 'reset') {
        cfg.whitelist = [];
        await db.antinuke.set(gid, cfg);
        return message.reply({ embeds: [E(GREEN, '✅ All whitelisted users have been cleared.')] });
      }

      return message.reply({ embeds: [E(RED, 'Whitelist sub-commands: `add`, `remove`, `show`, `reset`.')] });
    }
    ...
    if (sub === 'wlrole') {
      ...
      if (action === 'add') {
        ...
        if (!role) return message.reply({ embeds: [E(RED, 'Mention a role to whitelist.')] });
        if (cfg.wlRoles.includes(role.id)) return message.reply({ embeds: [E(YELLOW, `${role} is already whitelisted.`)] });
        cfg.wlRoles.push(role.id);
        await db.antinuke.set(gid, cfg);
        return message.reply({ embeds: [E(GREEN, `✅ ${role} has been whitelisted.`)] });
      }

      if (action === 'remove') {
        const role = message.mentions.roles.first();
        if (!role) return message.reply({ embeds: [E(RED, 'Mention a role to remove from the whitelist.')] });
        cfg.wlRoles = cfg.wlRoles.filter(id => id !== role.id);
        await db.antinuke.set(gid, cfg);
        return message.reply({ embeds: [E(GREEN, `✅ ${role} has been removed from the whitelist.`)] });
      }

      if (action === 'reset') {
        cfg.wlRoles = [];
        await db.antinuke.set(gid, cfg);
        return message.reply({ embeds: [E(GREEN, '✅ All whitelisted roles have been cleared.')] });
      }

      return message.reply({ embeds: [E(RED, 'WL Role sub-commands: `add`, `remove`, `list`, `reset`.')] });
    }

    return message.reply({ embeds: [E(RED, 'Unknown sub-command. See `?help antinuke`.')] });
  },
};
