...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (action === 'config') {
      ...
      const embed = new EmbedBuilder().setColor(0x5865F2).setTitle('⚙️ Auto-Role Configuration')
        .addFields(
          { name: 'Humans', value: humanRole ? humanRole.toString() : 'Not set', inline: true },
    ...
    if (action === 'reset') {
      ...
      if (target === 'bots') {
        ...
        return message.reply({ embeds: [new EmbedBuilder().setColor(0x57F287).setDescription('✅ Bot auto-role cleared.')] });
      }
      if (target === 'humans') {
    ...
    if (action === 'bots') {
      ...
      if (!role) {
        await db.guildConfig.set(message.guild.id, { autoRoleBot: null });
        return message.reply({ embeds: [new EmbedBuilder().setColor(0xFEE75C).setDescription('The bot auto-role no longer exists. It has been cleared.')] });
      }
      return message.reply({ embeds: [new EmbedBuilder().setColor(0x5865F2).setTitle('🤖 Bot Auto-Role').setDescription(`Current bot auto-role: ${role}`)] });
    }
    ...
    if (action === 'humans') {
      ...
      if (!role) {
        await db.guildConfig.set(message.guild.id, { autoRoleId: null });
        return message.reply({ embeds: [new EmbedBuilder().setColor(0xFEE75C).setDescription('The human auto-role no longer exists. It has been cleared.')] });
      }
    ...
    if (!role) {
      await db.guildConfig.set(message.guild.id, { autoRoleId: null });
      return message.reply({ embeds: [new EmbedBuilder().setColor(0xFEE75C).setDescription('The previously set auto-role no longer exists. It has been cleared.')] });
    }
    return message.reply({ embeds: [new EmbedBuilder().setColor(0x5865F2).setTitle('Auto-Role').setDescription(`Current auto-role: ${role}`)] });
  },
};
