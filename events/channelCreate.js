...
module.exports = {
  ...
  async execute(channel, client) {
    try {
      ...
      // Use the channel creator from audit log
      // Fetch up to 100 entries (Discord's max per request — effectively unlimited
      // within a single page) and find the one whose target is this channel, so
      // rapid-fire channel creations don't get the wrong executor attributed.
      const logs = await guild.fetchAuditLogs({ limit: 100, type: AuditLogEvent.ChannelCreate }).catch(() => null);
      if (!logs || !logs.entries.size) return;
      let entry = null;
      for (const e of logs.entries.values()) {
        if (e.targetId === channel.id) { entry = e; break; }
      }
      ...
      logger.warn('channelCreate anti-nuke error', e.message);
    }
  },
};
